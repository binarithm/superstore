...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    UPDATE_MULTI_FORM_FIELDS (state, payload) {
      state.form.fields[payload.inputIndex][payload.newArrayIndex][payload.multipleInputIndex] = payload.event
    },
    PUSH_MULTI_FORM_FIELDS (state, payload) {
      if (!Array.isArray(state.form.fields[payload.field])) {
        state.form.fields[payload.field] = []
      }
      state.form.fields[payload.field].push(payload.value)
    },
    SPLICE_MULTI_FORM_FIELDS (state, payload) {
      state.form.fields[payload.field].splice(payload.index, 1)
    },
